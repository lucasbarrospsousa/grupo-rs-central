...
func request(path: String, fields: Dictionary = {}) -> Dictionary:
	if not ORIGINS.has(branch):return {"ok":false,"message":"Base não reconhecida."}
	...
	if response[0]!=HTTPRequest.RESULT_SUCCESS or code<200 or code>=300:return {"ok":false,"code":code,"message":"Consulta indisponível (HTTP %d)." % code}
	var decoder:=JSON.new()
	if decoder.parse(response[3].get_string_from_utf8())!=OK:return {"ok":false,"message":"A API não retornou JSON válido."}
...
func login() -> Dictionary:
	...
	if str(credentials.get("username",""))=="" or str(credentials.get("password",""))=="":return {"ok":false,"message":"Configure o acesso desta base nas Configurações."}
	...
	return {"ok":token!="","message":"A API não confirmou a autenticação desta base."}
...
func lookup(serial: String) -> Dictionary:
	...
	if payload is Dictionary and host._grupo_rs_api_response_has_explicit_failure({"ok":true,"body":JSON.stringify(payload)}):return {"ok":false,"message":"A API recusou a consulta."}
	...
	if client=="":
		...
		if not owner.ok:return {"ok":false,"plate":plate,"client":"","message":owner.message}
		client=owner.client
...
func lookup_client(serial:String,plate:String) -> Dictionary:
	if not is_instance_valid(portal):
		...
		if not first.ok:return {"ok":false,"message":"Consulta do cliente indisponível no portal."}
		var auth:Dictionary=await portal.request("/login.php",{"usuario":credentials.get("username",""),"senha":credentials.get("password","")})
		if not auth.ok or int(auth.get("code",0)) not in [200,302,303]:return {"ok":false,"message":"Acesso ao cliente não confirmado no portal."}
	var response:Dictionary=await portal.request("/cadastro/veiculos_listar.php?busca=%s&status=Todos" % serial.uri_encode())
	if not response.ok or response.get("code",0)!=200:return {"ok":false,"message":"Falha ao conferir o cliente no portal."}
	var matches:Array=[]
