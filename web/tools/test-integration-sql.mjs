...
const service={equipmentPortal:async()=>({phone:'11900000000'}),binding:async()=>({ok:!changed,plate:'ABC1D23',client:'Synthetic',source:'synthetic'}),prepareLink:async(branch,serial,plate)=>({confirmed,serial,plate,payload:{synthetic:true}}),createLink:async()=>{calls++;throw Error('synthetic timeout');}};
...
const testLease=await admin.connect();const leased=(await testLease.query("select pg_try_advisory_lock(hashtext('central-web-sms-bridge')) as ok")).rows[0].ok;
try{
 if(!leased)throw Error('Pare a ponte antes dos testes sintéticos de fila.');
 ...
 check((await call('integrations/sms?branch=imperatriz','POST',{})).status===400,'SMS requires explicit confirmation');
 const smsId=await device('Estoque');await admin.query('update central_homologacao.devices set serial=$2 where id=$1',[smsId,'024999999']);
 const smsBody={id:smsId,version:1,phone:'+5511900000000',command:'SYNTHETIC-NO-SEND',confirmed:true};
 const queued=await call('integrations/sms?branch=imperatriz','POST',smsBody);check(queued.status===200&&queued.data.state==='queued','SMS queued in SQL without contacting Galaxy');
 const repeat=await call('integrations/sms?branch=imperatriz','POST',smsBody);check(repeat.data.id===queued.data.id,'duplicate confirmation reuses durable SMS operation');
 check((await call('integrations/reconcile?branch=imperatriz','POST',{id:queued.data.id})).data.state==='queued','SMS reconcile only reads recorded state');
 await admin.query('delete from central_homologacao.remote_operations where id=$1',[queued.data.id]);
 ...
 server.closeAllConnections();await new Promise(r=>server.close(r));await runtime.end();if(leased)await testLease.query("select pg_advisory_unlock(hashtext('central-web-sms-bridge'))");testLease.release();await admin.end();
}
