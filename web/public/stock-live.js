...
export function createStockLive({repo,branch,draw,showModal}){
 ...
 return{refresh,decorate:rows=>rows.map(r=>{const data=samples.get(r.id);if(!data)return {...r,communication:'Consultando…',connectivity:'Consultando…'};return{...r,communication:communication(data.location),connectivity:data.chip?.ok?data.chip.connectivity||'Não informado':'Consulta pendente',phone:data.equipment?.phone||r.phone,iccid:data.equipment?.iccid||r.iccid};}),
