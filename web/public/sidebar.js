...
export function mountSidebar({route,icon,navigate,logout,username}) {
 ...
 if(expandedGroup===undefined)expandedGroup=activeGroup||null;
 const userIcon='<svg class="icon" aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="8" r="4"/><path d="M4 22v-2a8 8 0 0 1 16 0v2"/></svg>';
 const link=([id,label,glyph])=>`<a href="#${id}" data-nav="${id}" ${id===route?'aria-current="page"':''}>${id==='users'?userIcon:icon(glyph)}<span>${label}</span></a>`;
 const group=(id,label,glyph)=>`<button type="button" data-group="${id}" aria-controls="nav-${id}" aria-expanded="${id===expandedGroup}" class="nav-group ${id===activeGroup?'contains-active':''}">${icon(glyph)}<span>${label}</span>${icon('down')}</button><div id="nav-${id}" class="nav-children" ${id===expandedGroup?'':'hidden'}>${groups[id].map(link).join('')}</div>`;
 sidebar.innerHTML=`<div class="brand"><img src="logo.png" alt="Grupo RS"><div><small>GRUPO RS</small><b>CENTRAL</b></div></div><div class="nav-label">CENTRAL DE OPERAÇÕES</div><nav aria-label="Navegação principal">${link(['overview','Visão geral','home'])}${group('equipment','Equipamentos','box')}${link(['maintenance','Manutenções','tool'])}${group('tracking','Rastreamento','map')}${link(['warehouse','Armazém','fork'])}${link(['sms','Painel SMS','mail'])}${link(['settings','Configurações','settings'])}${username==='lucasabm'?link(['users','Usuários e permissões','settings']):''}</nav><div class="bottom"><small></small><a href="#exit" id="exit">${icon('out')}<span>Sair</span></a></div>`;
 ...
 sidebar.querySelectorAll('[data-group]').forEach(b=>b.onclick=()=>{const open=b.getAttribute('aria-expanded')!=='true';expandedGroup=open?b.dataset.group:null;sidebar.querySelectorAll('[data-group]').forEach(other=>{const expanded=other===b&&open;other.setAttribute('aria-expanded',String(expanded));sidebar.querySelector('#nav-'+other.dataset.group).hidden=!expanded;});});
 const nav=sidebar.querySelector('nav');nav.scrollTop=scrollTop;nav.onscroll=()=>{scrollTop=nav.scrollTop;};
