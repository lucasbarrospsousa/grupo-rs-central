...
export function mountLinking({branch,branchName,icon,showModal,notify}) {
 const esc=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
 // Dedicated synthetic fixtures; never write to the stock adapter or a remote API.
 const rows=branch==='imperatriz'?Array.from({length:12},(_,i)=>({serial:`024800${String(i+1).padStart(3,'0')}`,model:i===0?'V7.2.2/7.1.6':'RS300',status:i<8?'Manutenção':'Reserva'})):[];
 ...
 const group=document.createElement('div');group.className='stock-nav-group';group.innerHTML=`${icon('box')}<b>Equipamentos</b><span>⌃</span>`;document.querySelector('[data-route="stock"]').before(group);
 for(const route of ['stock','link','bulk'])document.querySelector(`[data-route="${route}"]`).classList.add('stock-subnav');
 ...
 document.querySelector('#link-list').innerHTML=visible.map(r=>`<button class="link-row ${selected?.serial===r.serial?'selected':''}" data-link-serial="${r.serial}" aria-pressed="${selected?.serial===r.serial}"><span>${r.serial}<small>${r.model}</small></span><span class="link-state ${r.status==='Reserva'?'reserve':''}">${r.status}</span></button>`).join('')||`<p class="link-empty">${branch!=='imperatriz'?'Vinculação disponível inicialmente para Imperatriz.':'Nenhum aparelho encontrado com esse filtro.'}</p>`;
