...
function confirmAction(title, description, apply) {
  showModal(title, `<p>${escape(description)}</p><p class="muted">${repo.real?'Esta ação será registrada na Central online.':'Simulação: esta ação altera somente os dados de teste desta página.'}</p><div class="form-actions">${button('Cancelar', 'cancel')}${button(repo.real?'Confirmar operação':'Confirmar simulação', 'confirm', 'primary')}</div>`, 'compact');
  on('cancel', 'click', () => modal.close()); on('confirm', 'click', () => safe(async () => { const b=document.querySelector('#confirm');b.disabled=true;try{await apply();modal.close();render();}finally{b.disabled=false;} }));
}
function login() {
  if(repo.real){
    app.innerHTML=`<main class="login"><section class="login-brand"><img src="logo.png" alt="Grupo RS"><small>CENTRAL DE OPERAÇÕES</small><h1>Sua operação.<br>Mais perto de você.</h1><p>Equipamentos, estoque e filiais em um só lugar.<br>Clareza para cada decisão do seu dia.</p><p>Uma central. Todas as suas bases.<br>Acesso organizado por filial.</p><small>DESENVOLVIDO POR SideraCode</small></section><section class="login-main"><span class="pill">${mode==='production'?'CENTRAL ONLINE • ACESSO EXCLUSIVO':'HOMOLOGAÇÃO • SQL'}</span><p>Bem-vindo de volta</p><h1>Acesse sua central</h1><p>Informe seu usuário e senha para continuar.</p><form id="real-login"><label class="field">Usuário<input name="username" autocomplete="username" placeholder="Seu usuário" required></label><label class="field">Senha<input id="login-password" name="password" type="password" autocomplete="current-password" placeholder="Sua senha" required></label><button type="button" id="reveal-password">Mostrar senha</button><label class="field"><span><input name="remember" type="checkbox" checked> Manter conectado neste computador</span></label><button class="primary" type="submit">Entrar no sistema →</button><p id="login-error" role="alert"></p></form><p class="muted">${mode==='production'?'Acesso exclusivo. Suas alterações são salvas na Central online.':'Acesso restrito a usuários autorizados. Cópia isolada de testes.'}</p></section></main>`;
    on('reveal-password','click',e=>{const p=document.querySelector('#login-password');p.type=p.type==='password'?'text':'password';e.target.textContent=p.type==='password'?'Mostrar senha':'Ocultar senha';});
    on('real-login','submit',async e=>{e.preventDefault();const btn=e.target.querySelector('[type=submit]');btn.disabled=true;try{const f=new FormData(e.target);await repo.login(f.get('username'),f.get('password'),f.has('remember'));await enterSql();}catch(error){document.querySelector('#login-error').textContent=error.message;btn.disabled=false;}});
    return;
  }
...
function warehouse() {
  ...
  const draw = () => {
    ...
    if (state.warehouseTab === 'movements') target.innerHTML = movementsTable(repo.movements.filter(m=>[m.serial,m.type,m.destination].some(v=>String(v).toLowerCase().includes(text))).slice(warehousePage*warehousePageSize,(warehousePage+1)*warehousePageSize));
      ...
      target.innerHTML = `<table><thead><tr><th>Selecionar</th><th>${state.warehouseTab === 'device' ? 'Número de série' : 'ICCID'}</th><th>Recebido em</th><th>Situação</th><th>Remover</th></tr></thead><tbody>${rows.slice(warehousePage*warehousePageSize,(warehousePage+1)*warehousePageSize).map(d => `<tr><td><input type="checkbox" aria-label="Selecionar ${d.serial}" data-item="${d.id}" ${state.selected.has(d.id) ? 'checked' : ''} ${d.status !== 'Disponível' ? 'disabled' : ''}></td><td><strong>${d.serial}</strong></td><td>${d.received}</td><td><span class="pill ${d.status === 'Disponível' ? 'green' : 'blue'}">${d.status}</span></td><td><button data-remove="${d.id}" class="row-action danger-text" aria-label="Remover ${d.serial}" ${d.status !== 'Disponível' ? 'disabled' : ''}>${icon('close')}</button></td></tr>`).join('') || '<tr><td colspan="5" class="empty">Nenhum item encontrado.</td></tr>'}</tbody></table>`;
    }
    document.querySelector('#warehouse-pagination')?.remove();
    const visibleTotal=state.warehouseTab==='movements'?repo.movements.filter(m=>[m.serial,m.type,m.destination].some(v=>String(v).toLowerCase().includes(text))).length:repo.warehouse.filter(d=>d.kind===state.warehouseTab&&(!status||d.status===status)&&d.serial.includes(text)).length;
    const pager=document.createElement('div');pager.id='warehouse-pagination';pager.className='warehouse-pagination';pager.innerHTML=`<span>${visibleTotal} registros • página ${warehousePage+1}</span><div><button id="warehouse-prev" ${warehousePage===0?'disabled':''}>‹</button><button id="warehouse-next" ${(warehousePage+1)*warehousePageSize>=visibleTotal?'disabled':''}>›</button></div>`;target.after(pager);
    document.querySelector('#warehouse-prev').onclick=()=>{warehousePage--;draw();};document.querySelector('#warehouse-next').onclick=()=>{warehousePage++;draw();};
    ...
    document.querySelectorAll('[data-remove]').forEach(b => b.onclick = () => confirmAction('Remover do Armazém?', repo.real?'O item será removido do armazém, com registro de auditoria.':'O item de demonstração será retirado da lista e a remoção ficará no histórico.', async () => { await repo.removeWarehouse(b.dataset.remove); state.selected.delete(b.dataset.remove); }));
  ...
  document.querySelectorAll('[data-tab]').forEach(b => b.onclick = () => { state.warehouseTab = b.dataset.tab; state.selected.clear(); render(); });
...
function newWarehouseItem() {
  ...
  if(repo.real){document.querySelector('#warehouse-form .muted').textContent='Cadastro no armazém da Central.';document.querySelector('#save-item').textContent='Salvar';}
  ...
  on('new-serial','input',validate); on('warehouse-form','submit',e => { e.preventDefault(); safe(async () => { await repo.addWarehouse(kind, document.querySelector('#new-serial').value); modal.close(); render(); notify(repo.real?'Aparelho cadastrado.':'Aparelho demonstrativo cadastrado.'); }); });
}
function maintenance() { mountMaintenance({repo,branch:state.branch,icon,showModal,reportForm,render}); }
function reportForm() {
  const branch = state.branch; const vehicles = repo.real?repo.list(branch).filter(d=>d.status==='Instalado'):repo.vehicles.filter(v => v.branch === branch);
  showModal('Relatório de manutenção', `<form id="report-form"><label class="field">Cliente / veículo<select name="vehicle" required>${options(vehicles.map(v => [v.id, `${v.client} • ${v.plate} • ${v.serial}`]), '')}</select></label><div class="form-grid"><label class="field">Motivo<select name="reason" id="report-reason">${options(['Sem comunicação','Localização errada','Troca de aparelho'].map(s => [s,s]), 'Sem comunicação')}</select></label><label class="field">Meio<select name="medium">${options(['App de rastreamento','Suporte do rastreio','Consultor informou'].map(s => [s,s]), 'App de rastreamento')}</select></label></div><label class="field" id="replacement-field" hidden>Aparelho que entrará no veículo<select name="replacement"><option value="">Selecionar aparelho disponível</option>${options(repo.list(branch).filter(d => d.status === 'Estoque'||branch!=='imperatriz'&&d.status==='Reserva').map(d => [d.id,d.serial]), '')}</select><small>${repo.real?'A troca registra o atendimento e baixa somente o estoque SQL da Central. O vínculo na plataforma e o aparelho de chegada não serão alterados.':'A troca registrada dará baixa somente no estoque demonstrativo da Central.'}</small></label><label class="field">Relato / observações<textarea name="notes" rows="4" maxlength="2000"></textarea></label><div class="form-actions"><button type="submit" class="primary">Salvar relatório</button></div></form>`, 'medium');
  on('report-reason','change',e => document.querySelector('#replacement-field').hidden = e.target.value !== 'Troca de aparelho');
  on('report-form','submit',e => { e.preventDefault(); const submit=e.target.querySelector('[type=submit]');if(submit.disabled)return;submit.disabled=true;safe(async () => { try{const data=Object.fromEntries(new FormData(e.target));if(data.reason!=='Troca de aparelho')data.replacement='';if(repo.real){data.vehicleVersion=repo.list(branch).find(r=>r.id===data.vehicle)?.version;if(data.replacement)data.replacementVersion=repo.list(branch).find(r=>r.id===data.replacement)?.version;}await repo.saveReport({ ...data, branch, id: e.target.dataset.requestId||(e.target.dataset.requestId=crypto.randomUUID()) }); modal.close(); render(); notify(repo.real?'Atendimento salvo.':'Relatório salvo na demonstração.');}finally{submit.disabled=false;} }); });
}
