...
function warehouse() {
  ...
  const draw = () => {
    ...
    if (state.warehouseTab === 'movements') target.innerHTML = movementsTable(repo.movements.filter(m=>[m.serial,m.type,m.destination].some(v=>String(v).toLowerCase().includes(text))).slice(warehousePage*warehousePageSize,(warehousePage+1)*warehousePageSize));
      ...
      target.innerHTML = `<table><thead><tr><th>Selecionar</th><th>${state.warehouseTab === 'device' ? 'Número de série' : 'ICCID'}</th><th>Recebido em</th><th>Situação</th><th>Remover</th></tr></thead><tbody>${rows.slice(warehousePage*warehousePageSize,(warehousePage+1)*warehousePageSize).map(d => `<tr><td><input type="checkbox" aria-label="Selecionar ${d.serial}" data-item="${d.id}" ${state.selected.has(d.id) ? 'checked' : ''} ${d.status !== 'Disponível' ? 'disabled' : ''}></td><td><strong>${d.serial}</strong></td><td>${d.received}</td><td><span class="pill ${d.status === 'Disponível' ? 'green' : 'blue'}">${d.status}</span>${d.usage_device_serial?`<small style="display:block">Aparelho ${escape(d.usage_device_serial)}<br>Detectado em ${new Date(d.usage_detected_at).toLocaleString('pt-BR')}</small>`:''}</td><td><button data-remove="${d.id}" class="row-action danger-text" aria-label="Remover ${d.serial}" ${d.status !== 'Disponível' ? 'disabled' : ''}>${icon('close')}</button></td></tr>`).join('') || '<tr><td colspan="5" class="empty">Nenhum item encontrado.</td></tr>'}</tbody></table>`;
    }
