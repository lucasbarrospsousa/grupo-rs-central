...
export async function integrationRoute({req,res,url,pool,user,readBody,service=integrations}){
 ...
 try{
  ...
  const remote=await service.binding(branch,current.serial);const manual=body.manual===true&&['stock','review'].includes(body.confirmedCategory)&&remote.category===body.confirmedCategory;if((!remote.ok&&!manual)||(remote.plate||'')!==(body.plate||'')||(remote.client||'')!==(body.client||''))throw fail(409,'Vínculo mudou ou não foi confirmado. Analise novamente.');
