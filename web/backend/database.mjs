...
export function connectionOptions({ admin = false } = {}) {
  const runtime = admin ? null : process.env.CENTRAL_DB_USER ? {user:process.env.CENTRAL_DB_USER,password:process.env.CENTRAL_DB_PASSWORD} : JSON.parse(readFileSync(new URL('runtime-db.json', privateRoot), 'utf8'));
  return {
    ...
    ssl: { rejectUnauthorized: true, ca: process.env.CENTRAL_DB_CA || readFileSync(new URL('supabase-ca.crt', privateRoot), 'utf8') },
