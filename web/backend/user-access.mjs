...
export async function manageUsers(pool,access,method,id,body){
 ...
 const c=await pool.connect();try{await c.query('BEGIN');
  if(method==='PATCH'){const target=(await c.query('select username from central_homologacao.users where id=$1 for update',[id])).rows[0];if(!target)throw fail(404,'Usuário não encontrado.');if(target.username==='lucasabm')throw fail(403,'O administrador principal é protegido.');await c.query('update central_homologacao.users set active=$2 where id=$1',[id,body.active]);if(body.password)await c.query('update central_homologacao.users set password_hash=$2 where id=$1',[id,passwordHash(body.password)]);
  }else{id=randomUUID();if(username==='lucasabm')throw fail(403,'Administrador protegido.');await c.query('insert into central_homologacao.users(id,username,password_hash,active) values($1,$2,$3,$4)',[id,username,passwordHash(body.password),body.active]);}
