...
export async function remoteAction({action,body,branch,user,role,pool,service}){
 if(action==='sms')throw fail(503,'SMS pausado até restabelecer a conexão com o Galaxy.');
 ...
 const existing=await scoped(pool,user,async c=>(await c.query("select * from central_homologacao.remote_operations where branch_id=$1 and kind=$2 and serial=$3 and state in ('prepared','submitted','pending')",[branch,action,device.serial])).rows[0]);if(existing)return reconcile(existing,{pool,user,service});
 let payload;
 if(action==='link'){
  ...
  const remote=await service.equipmentPortal(branch,device.serial);const digits=remote.phone.replace(/\D/g,''),phone='+'+(digits.startsWith('55')?digits:'55'+digits);if(!/^\+55[1-9]\d{9,10}$/.test(phone)||body.phone!==phone)throw fail(409,'Telefone não confirmado ou mudou. Consulte novamente.');
  const health=await gateway({action:'health'});if(!health.ok)throw fail(503,health.error||'Galaxy indisponível.');const now=Math.floor(Date.now()/1000);payload={version:2,branch,serial:device.serial,phone,command:body.command,command_mode:'custom',source_phone_snapshot:phone,apn_snapshot:'',standard_command_snapshot:'',status_snapshot:device.data.status,created_at:now,expires_at:now+7200};
 }
 ...
 const op=await scoped(pool,user,async c=>{const fresh=(await c.query('select version from central_homologacao.devices where id=$1 and deleted_at is null for update',[device.id])).rows[0];if(fresh?.version!==device.version)throw fail(409,'Cadastro mudou durante a revisão.');const row=(await c.query("insert into central_homologacao.remote_operations(id,user_id,branch_id,kind,serial,payload,state) values($1,$2,$3,$4,$5,$6,'submitted') returning *",[id,user.user_id,branch,action,device.serial,payload])).rows[0];await c.query('insert into central_homologacao.audit_events(branch_id,user_id,action,entity_id,details) values($1,$2,$3,$4,$5)',[branch,user.user_id,'PREPARE_'+action.toUpperCase(),id,{device:device.id}]);return row;});
 try{if(action==='link'&&!payload.confirmed)await service.createLink(branch,payload.payload);if(action==='sms')await gateway({action:'send',id,payload});}catch{/* Ambiguous response is reconciled by reads only. */}
...
async function reconcile(op,{pool,user,service}){
 ...
 let result;try{
  ...
  }else{const remote=await gateway({action:'read',id:op.id});if(!remote.ok)return{ok:false,pending:true,id:op.id,message:'Retorno do Galaxy pendente. Nenhum reenvio automático.'};if(!['received','sending','sent','delivered','failed','cancelled','expired'].includes(remote.state))throw fail(502,'Estado do gateway não reconhecido.');await scoped(pool,user,c=>c.query('update central_homologacao.remote_operations set state=$2,result=$3,updated_at=now() where id=$1',[op.id,remote.state,remote]));result={ok:true,id:op.id,state:remote.state};}
