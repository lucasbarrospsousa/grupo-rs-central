...
A posição da API abre a janela imediatamente. Para posições vindas da API, o painel mostra Consultando… e confirma o titular diretamente no portal em segundo plano, por série e identificação, mesmo quando já existe nome no contexto local. O mapa e seus controles continuam disponíveis. Nomes locais ou herdados da API não dispensam essa confirmação; um nome antigo não pode ser apresentado como verificado. Apenas o campo Cliente e a indicação de sua fonte são atualizados, sem reiniciar ou recentralizar o mapa. A opção de leitura web é respeitada. Falhas preservam a posição; respostas obsoletas por nova consulta, troca de filial ou fechamento são descartadas. Testes sintéticos cobrem atraso do portal, revalidação de nomes locais e descarte de resposta antiga.

## Prioridade do titular confirmado

A conferência web substitui também nomes preenchidos no contexto local. Enquanto consulta, o painel não apresenta o nome anterior como atual. Em falha, ausência ou ambiguidade, não reutiliza o nome antigo como confirmado. Posição, ignição, horário e tensão continuam com a origem API indicada. Um zero de tensão recebido da origem não é convertido em ausência ou substituído por valor inventado. Testes sintéticos cobrem nome antigo divergente e falha sem reutilização; consulta operacional de leitura verificou a correção, sem registrar dados pessoais no repositório.
